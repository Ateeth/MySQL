...
-- not null keyword , auto increment keyword , primary key
create table bands(
...
-- create another table album , foreign key
create table albums(
	id int not null auto_increment , 
	name varchar(255) not null , 
	release_year int , 
...
-- Add records into table bands 
insert into bands (name) 
	values ('Iron Maiden') ; 

-- Add multiple records at a time from table
insert into bands (name)
...
-- Select fixed number of records from table
select * from bands limit 2 ; 

-- Select a specific column from table
select name from bands ; 

-- Select records from table but as different column names
select id as 'ID' , name as 'Band Name' from bands ; 

-- Order by (sort) by a specific column - default ascending order
select * from bands order by name ; 

-- Order by (sort) by a specific column descending
select * from bands order by name desc ; 

-- Order by (sort) by a specific column ascending
...
-- Select unique records from database
select distinct name from albums ; 

-- use update query to modify records
update albums 
	set release_year = 1982
	where id = 1 ; 

-- Query records based on condition 
select * from albums 
	where release_year < 2000 ; 
	
-- wildcards to filter strings , or clause
-- In this example select names that contain er substring or band_id is 2
select * from albums
...
-- where statement to filter between 2 different valeus
select * from albums 
...
-- Join statement to join 2 tables together
-- Inner Join
select * from bands
...
-- Use table alias and combining group by and join 
select b.name as band_name , count(a.id) as num_albums
	from bands as b
	left join albums as a on b.id = a.band_id
	group by b.id ; 
	
-- Query condition where aggregate function and group by using Having not where
-- use having when querying after a group by
-- we can use where before group by and does not involve aggregate functions
select b.name as band_name , count(a.id) as num_albums
	from bands as b
	left join albums as a on b.id = a.band_id
	where b.name = 'Deuce'
...
-- get id of row where release year is null
update albums 
	set release_year = 1986
	where id = 4 ;
	
-- Question 8 : Insert a record forband and album
insert into bands (name)
	values ('favorite band name');

insert into albums (name, release_year, band_id)
	values ('favorite album name', 2022, 8);
	
-- Question 9 : Delete the Band and Album you added in Question 8
...
-- Question 11 : Select the longest Song off each Album
select
